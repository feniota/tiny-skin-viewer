...
const wgslPath = "src/lib/shader.wgsl";
await Deno.writeTextFile(wgslPath, wgsl);

const kb = (wgsl.length / 1024).toFixed(1);
console.log(`\n✅  ${wgslPath}`);
console.log(`    ${totalVertices} vertices, ${kb} KB, ${partCount} parts × ${CUBE.length} verts`);

// ── write .ts (string export for JSR/Deno compatibility) ──────

const tsContent = `// Auto-generated by scripts/generate-shader.ts
// TypeScript wrapper — exports the WGSL shader code as a default string.
// Consumers that bundle via Vite can import shader.wgsl directly with \`?raw\`.
...
const tsPath = "src/lib/shader.ts";
await Deno.writeTextFile(tsPath, tsContent);

const tsKb = (tsContent.length / 1024).toFixed(1);
console.log(`✅  ${tsPath}`);
console.log(`    ${tsKb} KB`);
